...
if(isset($postdata) && !empty($postdata))
{
    ...
    $sql = "UPDATE `data_pasien` SET `no_registrasi` = '$noRegistrasi', `tgl_registrasi` = '$tglRegistrasi', `nama` = '$newName', `tempat_lahir` = '$tempatLahir', `tanggal_lahir` = '$tanggalLahir', `jenis_kelamin` = '$newKelamin', `agama` = '$agama', `alamat` = '$alamat', `rtrw` = '$rtrw', `kelurahan` = '$kelurahan', `kecamatan` = '$kecamatan', `kabupaten` = '$kabupaten', `propinsi` = '$propinsi', `nomor_hp` = '$newPhone', `kewarganegaraan` = '$kewarganegaraan', `noktp` = '$noktp', `pendidikan` = '$pendidikan', `pekerjaan` = '$pekerjaan', `status_perkawinan` = '$statusPerkawinan', `tgl_pertama_masuk` = '$tglPertamamasuk', `cara_bayar` = '$caraBayar', `tujuan_kunjungan_pertama` = '$tujuanKunjunganpertama', `alergi` = '$alergi', `catatan` = '$catatan' WHERE `data_pasien`.`id` = $newId;";
