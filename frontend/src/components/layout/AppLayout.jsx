...
function SidebarItem({ to, label, icon: Icon, end, collapsed }) {
    return (
        <NavLink
            ...
            className={({ isActive }) =>
                cn(
                    'flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 group relative min-w-0',
        ...
        >
            {({ isActive }) => (
                <>
                    ...
                    <Icon size={18} className={cn('flex-shrink-0', collapsed && 'mx-0')} />
                    {!collapsed && <span className="truncate min-w-0">{label}</span>}
                    {!collapsed && (
                        <ChevronRight size={14} className="ml-auto opacity-0 group-hover:opacity-50 transition-opacity flex-shrink-0" />
                    )}
                </>
            )}
        </NavLink>
...
export default function AppLayout() {
    ...
    return (
        <div className="flex h-screen bg-slate-100 overflow-hidden text-slate-900">
            ...
            <aside
                className={cn(
                    'flex flex-col bg-slate-900 text-slate-300 border-r border-slate-800 transition-all duration-300 flex-shrink-0 z-20 h-full overflow-y-auto min-w-0',
                    sidebarOpen ? 'w-[240px] min-w-[240px]' : 'w-16 min-w-[16px]'
                )}
            >
                {/* Logo */}
                <div className="flex items-center gap-3 h-16 px-4 border-b border-slate-800 flex-shrink-0 min-w-0">
                ...
                {/* User / Logout */}
                <div className="p-3 border-t border-slate-800 flex-shrink-0 min-w-0">
                    {sidebarOpen ? (
                        <div className="flex items-center gap-3 px-3 py-2 bg-slate-800/30 rounded-xl min-w-0">
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
                {/* Topbar */}
                <header className="h-16 bg-white border-b border-slate-200 px-6 flex items-center justify-between flex-shrink-0 shadow-sm z-10 min-w-0">
                    <button
                    ...
                    <div className="flex items-center gap-4 min-w-0">
                        <button className="text-slate-600 hover:text-slate-900 p-2 rounded-lg hover:bg-slate-100 relative">
                            <Bell size={20} />
                ...
                {/* Main content */}
                <main className="flex-1 min-w-0 overflow-auto bg-slate-100 p-6">
